...
def get_dingtalk_access_token_from_cache():
...
def save_dingtalk_access_token_to_cache(access_token):
    redis_client.set(access_token_key, access_token)
    return


def get_error_task_list_from_cache(owner, repo, sms_name):
...
def save_error_task_list_to_cache(owner, repo, sms_name, error_task_value):
    error_task_key = "{owner}/{repo}/{sms_name}/task/error".format(owner=owner, repo=repo, sms_name=sms_name)
